...
class TestTransactionsAPIClient(unittest.TestCase):
    ...
    def test_get_my_leagues_happy_path(self, mock_requests_get):
        ...
        response = UserFunctionsAPIClient.get_my_leagues(year=self.__TEST_YEAR, league_id=self.__TEST_LEAGUE_ID)
    ...
    def test_get_league_search_results_happy_path(self, mock_requests_get):
        ...
        mock_response = MockResponse(mock_dict, 200)
        mock_requests_get.return_value = mock_response
        response = UserFunctionsAPIClient.get_league_search_results(year=self.__TEST_YEAR,
